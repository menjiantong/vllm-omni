...
class LuminaFeedForward(nn.Module):
    ...
    def __init__(
        ...
        # Works for both tp_size=1 and tp_size>1
...
class SimpleFeedForward(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        return self.linear_2(F.silu(h1.float(), inplace=False).to(h1.dtype) * h2)
...
class TPAttention(nn.Module):
    ...
    def __init__(
        ...
        logger.info(
            "--my--debug-- TPAttention.__init__ called with: dim=%s, num_heads=%s, num_kv_heads=%s, eps=%s",
            dim,
            num_heads,
            num_kv_heads,
            eps,
...
class TransformerBlock(nn.Module):
    def __init__(
        ...
        # Use TPAttention for both tp_size=1 and tp_size>1
...
class Transformer2DModel(ModelMixin, ConfigMixin):
    ...
    def __init__(
        ...
        logger.info(
            "--my--debug-- Transformer2DModel.__init__ called with: patch_size=%s, in_channels=%s, hidden_size=%s, num_layers=%s, num_attention_heads=%s, num_kv_heads=%s, axes_dim_rope=%s",
            patch_size,
            in_channels,
            hidden_size,
            num_layers,
